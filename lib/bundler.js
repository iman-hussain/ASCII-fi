...
<body>
  ...
  <script>
    ...
    function fitAscii(pre, cols, rows) {
      // Measure one character width at a known font size
      var testSize = 100;
      var lh = 0.8; // match player line-height: 0.8em
      pre.style.fontSize = testSize + 'px';
      pre.style.lineHeight = lh + 'em';
      var span = document.createElement('span');
      span.textContent = 'M';
      span.style.visibility = 'hidden';
      pre.appendChild(span);
      var charW = span.getBoundingClientRect().width;
      var charH = testSize * lh;
      pre.removeChild(span);
      // Calculate the font size that fits both axes
      var vw = document.documentElement.clientWidth;
      var vh = document.documentElement.clientHeight;
      var fByW = (vw / (cols * (charW / testSize)));
      var fByH = (vh / (rows * (charH / testSize)));
      var ideal = Math.floor(Math.min(fByW, fByH) * 10) / 10;
      ideal = Math.max(2, ideal);
      pre.style.fontSize = ideal + 'px';
    }
    AsciiPlayer.fromCompressed(__ASCII_COMPRESSED__).then(function(player) {
      var container = document.getElementById('player-container');
      player.mount(container);
      var pre = container.querySelector('pre');
      if (pre) fitAscii(pre, player.width, player.height);
